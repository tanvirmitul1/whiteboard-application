...
const createDrawing = async (req, res) => {
  const { drawingTitle, shapes, canvasSize, userId, backgroundColor } =
    req.body;
...
const getTotalDrawCount = async (req, res) => {
  try {
    const totalDrawCount = await Whiteboard.countDocuments({});
    res.json({ totalDrawCount });
  } catch (error) {
    console.error("Error fetching total draw count:", error);
    res.status(500).json({ message: "Failed to fetch total draw count" });
  }
};
